...
export default function MetodosPagoAdminScreen() {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        <HeaderConfig />
        <FormCard
          title="Metodos de pago admin"
          subtitle="Se almacenan por administrador para dejar la base preparada mientras conectamos un backend especifico."
          fields={[
            {
              key: "alias",
              label: "Alias",
              ...
              placeholder: "Ej. Cuenta principal admin",
            },
            {
              key: "banco",
              label: "Banco",
        ...
        <TouchableOpacity style={styles.primaryButton} onPress={guardar}>
          <Text style={styles.primaryButtonText}>{editingId ? "Guardar cambios" : "Agregar metodo"}</Text>
        </TouchableOpacity>
        ...
        <Text style={styles.sectionTitle}>Metodos guardados</Text>

        {!metodos.length ? (
          <InfoCard
            title="No tienes metodos guardados"
            description="Agrega uno para dejar lista esta parte del modulo administrativo."
          />
        ) : (
          metodos.map((item) => (
            <View key={item.id} style={styles.methodCard}>
              ...
              <View style={styles.actions}>
                ...
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    const nuevos = metodos.filter((metodo) => metodo.id !== item.id);
                    await guardarMetodosPagoUsuario(userId, nuevos);
                    setMetodos(nuevos);
...
const styles = StyleSheet.create({
  ...
  methodTitle: { fontSize: 16, fontWeight: "700", color: "#111827" },
  methodText: { color: "#6b7280", marginTop: 4 },
